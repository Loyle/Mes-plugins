...
public class MyCommandExecutor implements CommandExecutor{
	...
	public MyCommandExecutor(KTP pl) {
	    this.plugin = pl;
	...
	private void loadCommands() {
		  ...
		  this.commands.put("stop", new ForceStopCommand(this.plugin));
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("KTP")) {
			if ((args == null) || (args.length < 1)) {
				player.sendMessage(ChatColor.YELLOW + "Plugin par LoyleCraft | version 1.0.0 | /KTP help for see all commands");
				return true;
			}
	      ...
	      if (!this.commands.containsKey(sub))
	      {
	        ...
	        player.sendMessage(ChatColor.GOLD + "Type /KTP help for help");
	        return true;
	      }
	      try
	      {
	      ...
	      catch (Exception e)
	      {
	        ...
	        player.sendMessage(ChatColor.BLUE + "Type /KTP help for help");
	      }
	      return true;
	}
	public void help(Player p) {
		p.sendMessage("/KTP <command> <arguments>");
